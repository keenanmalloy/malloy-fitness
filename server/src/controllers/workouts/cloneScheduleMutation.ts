...
export const cloneScheduleWorkoutMutation = async (
  ...
  try {
    ...
    const workoutQuery = `
      WITH
        workoutdata(name, description, category, created_by, workout_dt, task_order) AS (
          VALUES
            (
                '${workout.name}', 
                '${workout.description}', 
        ...
        INSERT INTO workouts (name, description, category, created_by, workout_dt, task_order)
          SELECT *
            FROM workoutdata
          RETURNING *
      `;
